# Claude Code Project Instructions — bm25-kotlin

## Project Overview

This is **bm25-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`bm25`](https://crates.io/crates/bm25).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/bm25/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/bm25>
