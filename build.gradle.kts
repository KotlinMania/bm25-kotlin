...
fun installProjectAndroidSdk(execOperations: ExecOperations) {
    ...
    if (!isProjectAndroidSdkInstalled()) {
        val missingPackageDirs = requiredAndroidSdkPackageDirs
            .filterNot { it.exists() }
            .joinToString(separator = "\n") { "  - $it" }
        throw GradleException("Android SDK setup finished but required package dirs are missing:\n$missingPackageDirs")
    }
...
kotlin {
    ...
    val xcf = XCFramework("Bm25")

    macosArm64 {
        binaries.framework { baseName = "Bm25"; xcf.add(this) }
    }
    iosArm64 {
        binaries.framework { baseName = "Bm25"; xcf.add(this) }
    }
    ...
    swiftExport {
        moduleName = "Bm25"
        flattenPackage = "io.github.kotlinmania.bm25"
    }

    android {
        namespace = "io.github.kotlinmania.bm25"
        compileSdk = 34
        minSdk = 24
    ...
    sourceSets {
        ...
        val commonTest by getting {
            dependencies {
                ...
                implementation("io.github.kotlinmania:csv-kotlin:1.3.1")
                implementation("io.github.kotlinmania:insta-kotlin:1.41.1")
            }
        }
...
mavenPublishing {
    ...
    coordinates(group.toString(), "bm25-kotlin", version.toString())

    pom {
        name.set("bm25-kotlin")
        description.set("Kotlin Multiplatform port of Michael-JB/bm25 - BM25 embedder, scorer, and search engine")
        inceptionYear.set("2026")
        url.set("https://github.com/KotlinMania/bm25-kotlin")
        ...
        scm {
            url.set("https://github.com/KotlinMania/bm25-kotlin")
            connection.set("scm:git:git://github.com/KotlinMania/bm25-kotlin.git")
            developerConnection.set("scm:git:ssh://github.com/KotlinMania/bm25-kotlin.git")
        }
    }
}
...
// itself produces no plain JVM classes for the java-kotlin extractor, so this
// task compiles commonMain with kotlinc against a JVM classpath of the same
// libraries commonMain depends on.
...
dependencies {
    ...
    codeqlSourceClasspath("io.github.kotlinmania:unicode-segmentation-kotlin-jvm:0.1.0")
}
...
val codeqlCompileJvm = tasks.register<JavaExec>("codeqlCompileJvm") {
    ...
    doFirst {
        ...
        if (sourceFiles.isEmpty()) {
            val sentinelFile = sentinelDir.get().asFile.resolve("io/github/kotlinmania/bm25/codeql/_CodeqlEmptySource.kt")
            sentinelFile.parentFile.mkdirs()
            sentinelFile.writeText(
                """
                // Auto-generated. Present so codeqlCompileJvm has at least
                // one Kotlin source to feed kotlinc.
                package io.github.kotlinmania.bm25.codeql
